...
//     const clientsByTaxNumber = clients.sort((a, b) => parseInt(a.taxNumber) - parseInt(b.taxNumber))
//     .map(client => client.id);
//     return clientsByTaxNumber;
// }

const listClientsIdsSortByTaxNumber = () => clients.sort((a, b) => parseInt(a.taxNumber) - parseInt(b.taxNumber))
    .map(client => client.id);
...
const sortClientsTotalBalances = () => {
    clientsWithBalance = clients.map(client => {
        const balances = [];
        accounts.filter(acc => acc.clientId === client.id && balances.push(acc.balance));
        return {[client.name]: balances.reduce((a, b) => a + b)}
    })
    return clientsWithBalance.sort((a,b) => Object.values(b) - Object.values(a)).map(clientName => Object.keys(clientName)[0]);
}
